...
def initialize_population(X, y, size, chromosome_length):
...
def fitness_score(population_x, population_y):
    scores = np.array([])
...
def selection(pop_x, pop_y, coef):
...
def crossover(parent_a_x, parent_a_y, parent_b_x, parent_b_y, num_children):
    offspring_x = []
    offspring_y = []
...
def mutation(offspring_x, offspring_y, mutation_rate):
    pop_nextgen_x = []
    pop_nextgen_y = []
...
def append_offspring(next_generation_x, next_generation_y, new_offspring_x, new_offspring_y):
...
# Main Function
def run_genetic_algorithm(X, y, population_size=10, chrom_len=10, n_generations=20, coef=0.5, mutation_rate=0.3):

    best_chromo = np.array([])
    best_score  = np.array([])

    next_generation_x, next_generation_y = initialize_population(X, y, size=population_size, chromosome_length=chrom_len)
...
model_test = AdaBoostSVM(C=50, gammaIni=100, myKernel='rbf')
#model_test = SVC()
...
model = AdaBoostSVM(C=50, gammaIni=100, myKernel='rbf')
